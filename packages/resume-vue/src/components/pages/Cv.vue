<script setup lang="ts">
  ...
  import ReferenceList from "../shared/ReferenceList.vue";
...
<template>
  <div v-if="loaded">
    <CommonHeader :subject="state.subject" v-if="state.subject">
      <RouterLink :to="{ name: 'resume' }" title="Click to view as Resume" class="no-print">Resume View</RouterLink>
    </CommonHeader>
    ...
    <ReferenceList :references="state.references" :businesses="state.businesses" />
  </div>
</template>